...
/**
 * Privacy Subsystem implementation for paygw_cashfree.
 *
...
class provider implements \core_privacy\local\metadata\null_provider, paygw_provider {
    ...
    /**
     * Export all user data for the specified payment record and context.
     *
     * @param \context $context Context
     * @param array $subcontext The location within the current context that the payment data belongs to
    ...
    public static function export_payment_data(\context $context, array $subcontext, \stdClass $payment) {
        ...
        $subcontext[] = get_string('gatewayname', 'paygw_cashfree');
        $record = $DB->get_record('paygw_cashfree', ['paymentid' => $payment->id]);
        ...
        $data = (object) [
            'orderid' => $record->cf_orderid,
            'paymentid' => $record->cf_paymentid,
    ...
    public static function delete_data_for_payment_sql(string $paymentsql, array $paymentparams) {
        ...
        $DB->delete_records_select('paygw_cashfree', "paymentid IN ({$paymentsql})", $paymentparams);
    }
}
